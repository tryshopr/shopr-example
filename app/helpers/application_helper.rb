module ApplicationHelper
  ...
  def link_to(name, options = nil, html_options = nil, &block)
    html_options[:class] = 'active' if html_options && @active_nav_item && @active_nav_item == html_options[:nav_item]
    super
  end
  ...
  def next_delivery_cutoff
    ...
    fourpm = (fourpm + 2.days).change(hour: 16, minute: 0) if fourpm.saturday?
    fourpm
  end
