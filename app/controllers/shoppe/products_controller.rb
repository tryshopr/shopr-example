module Shopr
  class ProductsController < shopr::ApplicationController
    ...
    def new
      @product = Shopr::Product.new
    end

    def create
      @product = Shopr::Product.new(safe_params)
      if @product.save
        redirect_to :products, flash: { notice: t('shopr.products.create_notice') }
      else
        render action: 'new'
      end
    end
    ...
    def update
      if @product.update(safe_params)
        redirect_to [:edit, @product], flash: { notice: t('shopr.products.update_notice') }
      else
        render action: 'edit'
      end
    end
    ...
    def destroy
      @product.destroy
      redirect_to :products, flash: { notice: t('shopr.products.destroy_notice') }
    end

    def import
      if request.post?
        if params[:import].nil?
          redirect_to import_products_path, flash: { alert: t('shopr.imports.errors.no_file') }
        else
          Shopr::Product.import(params[:import][:import_file])
