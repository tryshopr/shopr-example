class OrdersController < ApplicationController
  ...
  def status
    @order = Shopr::Order.find_by!(token: params[:token])
  end
  ...
  def change_delivery_service
    if current_order.delivery_service = current_order.available_delivery_services.select { |s| s.id == params[:delivery_service].to_i }.first
      ...
      respond_to do |wants|
        ...
        wants.json { render json: { status: 'error', message: 'InvalidDeliveryMethod' }, status: :unprocessable_entity }
      end
    end
  end
  ...
  def checkout
    ...
    if request.patch?
      ...
      redirect_to checkout_payment_path if @order.proceed_to_confirm
    else
  ...
  def payment
    @order = Shopr::Order.find(current_order.id)
    redirect_to checkout_confirmation_path if request.patch?
  end
  ...
  def confirmation
    ...
    if request.patch?
      begin
        ...
        current_order.payments.create(method: 'Credit Card', amount: current_order.total, reference: rand(10_000..19_999), refundable: true)
        session[:order_id] = nil
